...
**Voiceover transcript:** I kept making the same choice before every coding task: which model is enough? But a typo, a feature, and an authorization change carry very different risks. Dynamic Model Routing brings a simple, risk-aware policy to Codex and Claude Code. For Claude Code, validate the plugin, load it for the session, then invoke the namespaced skill. For Codex, add the local marketplace, install the plugin, and ask for Advise mode. The router classifies work as clerical, routine, sensitive, or escalation, while preserving your explicit choice. The pilot has seventeen regression tests, with green CI across Windows, macOS, and Linux. Version zero point one point one is free and MIT licensed. Try it, break it, and share the evidence on GitHub.

**Voice provenance:** Generated locally with the `af_heart` voice from the Apache-2.0-licensed Kokoro-82M model through the MIT-licensed `kokoro-onnx` runtime. No voice was cloned.
